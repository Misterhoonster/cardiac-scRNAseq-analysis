...
setwd("~/cardiac_seq_analysis/samples/2k23/")

# gather data
dataFolders<-as.matrix(read.csv("~/cardiac_seq_analysis/scripts/2k23/folders.txt",
                                header=FALSE))
dataFolders<-as.list(dataFolders[,1])

sampleNames <-as.matrix(read.csv("~/cardiac_seq_analysis/scripts/2k23/names.txt", 
                                 header=FALSE))
...
# save Raw object
save(choudhuryCells, file="~/cardiac_seq_analysis/data/2k23/raw_cells.Robj")

load("~/cardiac_seq_analysis/data/2k23/raw_cells.Robj")
...
VlnPlot(choudhuryCells, features = c("percent.mito"), y.max = 20)

choudhuryCells <- subset(choudhuryCells, percent.mito<5)
choudhuryCells <- subset(choudhuryCells, nCount_RNA > 1000 & nCount_RNA < 15000)

## saving
save(choudhuryCells, file="~/cardiac_seq_analysis/data/2k23/preprocess_cells.Robj")

### Choudhury cells seurat analysis

# load data
load("~/cardiac_seq_analysis/data/2k23/preprocess_cells.Robj")
...
Idents(choudhuryCells)<-choudhuryCells$SCT_snn_res.0.1
...
DimPlot(choudhuryCells, reduction = 'umap')
...
# ACTA1 vs. ACTC
# Group 1 (healthy): 771, 5708
# Group 2 (diseased): T1400, T1421
VlnPlot(choudhuryCells, features = c("ACTA1", "ACTC1"), idents = c("771_LV", "T1400_LV"))
VlnPlot(choudhuryCells, features = c("ACTA1", "ACTC1"), idents = c("5708_LV", "T1421_LV"))

VlnPlot(choudhuryCells, features = c("ACTA1", "ACTC1"), idents = c("T1302_LV", "T1309_Q"))
VlnPlot(choudhuryCells, features = c("ACTA1", "ACTC1"), idents = c("T1421_LV", "T1426_Psoas"))
